...
| **Parallel execution** | ~6 concurrent minions per engineer | Fan-out with `claude -p` in loops | Parallel agents via Task tool | Parallel backend + frontend within ticket, parallel tickets via `/create-stories` |
