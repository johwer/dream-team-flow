...
| **Deterministic nodes** | Core concept — git, lint, CI as code nodes, zero LLM tokens | Mentioned (hooks, CLI tools) | `quality-gate` hooks, formatters as PostToolUse | Same philosophy as Stripe: `quality-gate.sh`, `poll-ci-checks.sh`, `poll-ai-reviews.sh`, `allocate-ports.sh` as shell scripts (zero tokens). Plus ECC-style hooks that enforce automatically (migration guard, lock file guard, lint reminder, teammate idle gate, task completed gate). |
...
| Capability | Stripe | Claude Docs | ECC | DTF |
|---|---|---|---|---|
| **Self-learning** | Rule files updated manually by teams | CLAUDE.md iteration — "observe behavior, refine" | `continuous-learning-v2`: hooks capture patterns → instincts → `/evolve` to promote | ECC's automatic pattern detection (`analyze-patterns.sh` + `/evolve`) PLUS 5 additional paths: session retros, PR mining, Jira pushback, research articles, cross-session analysis. Most comprehensive learning loop of any system. |
...
### Dream Team Flow
- **Combines the best of all three** — Stripe's deterministic nodes + Claude Docs' verification philosophy + ECC's learning system, in one integrated package
- **Named multi-agent team** — 11 personas with structured communication, task ownership, deadlock detection
- **6 learning paths** — Most comprehensive learning loop of any system
- **Interactive user control** — Early triage + draft PR + user confirms at every gate → never auto-ships
