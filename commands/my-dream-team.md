...
4. **Collect all responses** and synthesize them into four categories, each with a **destination hint** for where the learning should eventually be applied (via [`/team-review`](team-review.md)):
...
8. **Based on user choice:**
   ...
   - Always append a session entry to `your project memory directory for `dream-team-learnings.md`` with destination hints so [`/team-review`](team-review.md) can route them later:
     ```
...
**IMPORTANT:** Do NOT run [`/workspace-cleanup`](workspace-cleanup.md) or remove the worktree/branch. The workspace cannot clean itself up because it's running inside its own worktree. The orchestrator session (from [`/create-stories`](create-stories.md)) handles all cleanup.
...
- The worktree must have a `.env` file with unique ports (generated by [`/workspace-launch`](workspace-launch.md) via `allocate-ports.sh`)
